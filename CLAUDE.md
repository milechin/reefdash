...
- `save.php` — PHP endpoint that receives POSTed JSON, backs up the existing tank_data.js, and writes the new version to disk
- `Fish_Tank_Log.xlsx` — source spreadsheet for historical data
- `Fish Tank Log - Display.csv` — exported CSV used to bulk-import historical data
...
- `display`, `qrt`, `laurens` — tank objects, each with:
  ...
  - `dose` — array of {date, dose} objects for AFR Dose (ml/day); stored in RAW so it persists via save.php
- `equipment` — array of warranty tracking objects
- `log` — array of recent activity entries

## Dashboard Features
- Tab navigation: Display Tank, QRT, Lauren's, Equipment, Log, Help
- Per-tank panel layout:
  - **Maintenance section**: 2-column row with "Last Water Change" card (has `+ LOG` button inline) and "Last Water Test" card
  - **Current Parameters section**: section label bar with `+ Log Test` button inline, then 8 parameter KPI cards
  - **Controls bar (dateBar)**: date pickers, presets (90d default, 6mo, 1yr, All), Water Changes toggle, All For Reef Dose toggle, ⚙ Targets
  - **Parameter Trends section**: 7 Chart.js charts: Temp, pH, Salinity, ALK, Calcium, Phosphate, Nitrate — all sharing a master x-axis
- Dose line: stepped coral line on pH/ALK/Calcium charts from Nov 2025 onward
- Water change annotations: blue dashed vertical lines via chartjs-plugin-annotation
- Target editor modal: editable min/max bands per parameter
...
- On save: inserts date into RAW[tankKey].waterChanges sorted
- On delete: removes date from waterChanges array
...
## Saving Pattern
save.php receives POST with {content: "const RAW = ..."}, validates it starts with "const RAW", backs up the existing file as tank_data.backup-{timestamp}.js, writes new tank_data.js. Returns {ok: true} or {ok: false, error: "..."}.

The dashboard's saveData() helper calls save.php, then:
...
## Nitrate Data Notes
- Display tank nitrate imported from `Fish Tank Log - Display.csv` (308 entries, 2021-01-11 to 2026-03-28)
